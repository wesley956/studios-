    ...
    <div>
      <TopHeading
        ...
        action={
          <div className="flex flex-wrap gap-3">
            <Link href="/app/agenda" className="rounded-2xl bg-primary px-5 py-3 text-white">
              Abrir agenda
            </Link>
            <Link href="/app/financeiro" className="rounded-2xl border border-border bg-white px-5 py-3">
              Ver financeiro
            </Link>
          </div>
        }
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          tone={summary.pendingRequests ? 'warning' : 'default'}
          hint="Pedidos aguardando aprovação"
        />
      </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          label="Serviço campeão"
          value={summary.topService}
          hint={summary.topServiceCount ? `${summary.topServiceCount} atendimentos concluídos no mês` : 'Sem vendas registradas ainda'}
        />
      </div>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1.15fr,0.85fr]">
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted">
                        {(item.services as { name?: string } | null)?.name || 'Serviço'} • {formatDateBR(item.appointment_date)} às{' '}
        ...
        <SectionCard
          ...
          description="Últimos pagamentos lançados no sistema."
          action={
            <Link href="/app/financeiro" className="text-sm text-primary">
              Abrir financeiro
            </Link>
          }
        >
                ...
                <div key={payment.id} className="rounded-2xl border border-border p-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="mt-1 text-xs text-muted">Cobrança: {currencyBRL(payment.final_amount || 0)}</p>
