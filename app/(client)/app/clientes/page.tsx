      ...
      <div className="mb-6 grid gap-4 md:grid-cols-[1fr,340px]">
        ...
        <SectionCard title="Resumo" description="Visão rápida da sua base cadastrada.">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="rounded-2xl bg-[#FBF7F3] p-4">
              <p className="text-muted">Com histórico</p>
              <p className="mt-1 text-2xl font-semibold">{[...appointmentMap.values()].filter(Boolean).length}</p>
            </div>
          </div>
        </SectionCard>
      </div>
                  ...
                  <summary className="flex cursor-pointer list-none flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-muted">
                        Aniversário: {customer.birthday ? formatDateBR(customer.birthday) : 'Não informado'} • Faturamento:{' '}
                        {currencyBRL(paymentMap.get(customer.id) || 0)}
                      </p>
                  ...
                  <form action={handleUpdateCustomer} className="mt-5 grid gap-4 border-t border-border pt-5">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <Field label="Nome completo">
                        <Input name="fullName" defaultValue={customer.full_name} />
                      </Field>
