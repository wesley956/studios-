...
import { appointmentApprovedMessage, buildWhatsappUrl } from '@/lib/whatsapp';
        ...
        {requests?.length ? requests.map((item) => (
          <details key={item.id} className="rounded-[1.5rem] border border-border bg-white p-6 shadow-sm open:bg-[#FCFAF7]">
            ...
            <div className="mt-5 rounded-[1.5rem] border border-border bg-[var(--theme-surface-alt)] p-4">
              ...
              <p className="mt-1 text-sm text-muted">Use depois de aprovar para enviar uma confirmação manual com tudo preenchido.</p>
              <a
                href={buildWhatsappUrl(item.customer_phone, appointmentApprovedMessage({
                  businessName: business.business_name,
                  customerName: item.customer_name,
                  serviceName: (item.services as { name?: string } | null)?.name || null,
                  date: item.approved_date || item.requested_date,
                  time: item.approved_time || item.requested_time,
